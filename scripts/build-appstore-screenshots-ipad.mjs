#!/usr/bin/env node
// App Store iPad pazarlama görselleri üretici (13" iPad, 2064x2752).
// Kaynak: /tmp/ipad-raw/0X-*.png (simctl ile çekilen ham iPad ekranları)
// Çıktı: docs/screenshots/appstore-ipad/0X-*.png  (2064x2752, alpha yok)
// iPhone sürümüyle aynı marka dili (mint pastel arka plan + çerçeve + Türkçe başlık).
//
// Kullanım: node scripts/build-appstore-screenshots-ipad.mjs
...
const SRC = "/tmp/ipad-raw";
const OUT = join(ROOT, "docs", "screenshots", "appstore-ipad");
...
function html(screen, b64) {
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  .blob { position:absolute; border-radius:50%; background:#ffffff; opacity:.16; filter:blur(3px); }
  .b1 { width:840px; height:840px; top:-230px; left:-260px; }
  .b2 { width:560px; height:560px; top:180px; right:-220px; opacity:.12; }
  .b3 { width:1100px; height:1100px; bottom:-520px; left:50%; transform:translateX(-50%); opacity:.10; }
  .title {
    position:relative; text-align:center; color:#0B332D;
    font-weight:800; font-size:128px; line-height:1.05; letter-spacing:-2.5px;
  }
  .sub {
    position:relative; text-align:center; color:#1C5A4E;
    font-weight:500; font-size:56px; line-height:1.3; letter-spacing:-.5px;
    margin-top:38px; max-width:1500px;
  }
  .frame {
    position:relative; margin-top:104px; width:1400px;
    background:#0A0A0C; border-radius:66px; padding:16px;
    box-shadow:0 56px 120px rgba(8,46,40,.40), 0 10px 30px rgba(8,46,40,.22);
  }
  .screen { display:block; width:100%; border-radius:50px; overflow:hidden; }
...
const tmp = mkdtempSync(join(tmpdir(), "appstore-ipad-"));
...
for (const s of SCREENS) {
  ...
  await shoot(htmlPath, rawPng, join(tmp, "profile-" + i));
